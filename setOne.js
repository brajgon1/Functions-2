...
// const buddy = (para1, para2) => console.log(`The first parameter is ${para1}. The second parameter is ${para2}`);
...
const greeting = (firstName, lastName, fn) => {
    fn(`${firstName} ${lastName}`) 
}
// INVOKE 'GREETING FUNCTION' HERE

greeting('Brandon', 'Gonzalez', (fullName) => {
    console.log(`Hello, my full name is ${fullName}`)
})
//the cb has been invoked with myName sothe anonymous function should look like this

// (`${firstName} ${lastName}`) => console.log(`Hello, my full name is ${`${firstName} ${lastName}`}`)
...
const pricesPlusTax = (arr, callbackPrice) => {
    for (let i = 0; i < arr.length; i++) {
        totalCost.push((arr[i] * .20).toFixed(2))
    }
    callbackPrice(totalCost)
}
...
timesFour(4)
// I had to make sure the timesFour was a variable that invoked the multiplyingFactory function in it and in the parenteses is the first paramenter for the first function
//** */
...
// const timesFour = multiplyingFactory(5)
timesFour(4)
//This will console log the number 20
